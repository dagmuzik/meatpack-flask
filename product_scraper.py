...
def get_kicks_products(talla="9.5", min_price=0, max_price=99999):
    ...
    try:
        ...
        for item in items:
            ...
            precio_final = item.select_one(".special-price .price") or item.select_one(".price")
            precio_original = item.select_one(".old-price .price")

            try:
                precio_final = float(precio_final.text.replace("Q", "").replace(",", "").strip())
            except:
                precio_final = 0.0

            if precio_original:
                try:
                    precio_original = float(precio_original.text.replace("Q", "").replace(",", "").strip())
                except:
            ...
            # No muestra tallas individuales, así que asumimos todas disponibles
            if talla not in nombre and talla not in url_producto:
                continue
